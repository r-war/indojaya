 ...
 */
define('HTTP_UPLOAD_DEFAULT_CHMOD', 0777);
 ...
 * @access   public
 */
class Upload extends HTTP_Upload_Error
{
    /**
...
class HTTP_Upload_File extends HTTP_Upload_Error
{
    ...
    function _evalValidExtensions()
    {
        ...
        if ($this->_extensions_mode == 'deny') {
        	foreach ($exts as $ext)
        	{
        		if(strcasecmp($this->getProp('ext'),$ext)==0) return false;
        	}
        	return true;
        // mode == 'accept'
        } else {
        	foreach ($exts as $ext)
        	{
        		if(strcasecmp($this->getProp('ext'),$ext)==0) return true;
        	}
        	return false;
        }
