...
class DefaultInterceptor extends AbstractInterceptor 
{
	public function doProcessInteceptor()
	{
		...
		$view = [
			# show latest news
			'latest_news' => NewsPeer::getByType('news', $oParam, null, -1, $oPager, 3),
			'interceptor' => $this
		];
	...
	public function menu($omod = null, $menu = null, $class = "have-submenu") {
		if ($menu == null) {
		...
		foreach ($menu as $item) {
			...
			if ( count($submenu) > 0 ) {
				$html .= '<li class="'. $class .'">';
				$html .= 		'<a '. $new_tab .' href="'. $item->getUrl( $omod ) .'"><span>'. $item->getName() .'</span></a>';
				$html .= $this->menu($omod, $submenu, "have-subsubmenu");
			}
			else {
				$html .= '<li>';
				$html .= 		'<a '. $new_tab .' href="'. $item->getUrl( $omod ) .'"><span>'. $item->getName() .'</span></a>';
