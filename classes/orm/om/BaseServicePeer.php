...
abstract class BaseServicePeer {
	...
	const CLASS_DEFAULT = 'orm.Service';
	...
	const SHORT_DESCRIPTION = 'service.SHORT_DESCRIPTION';
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(ServicePeer::INDEX);

		$criteria->addSelectColumn(ServicePeer::NAME);

		$criteria->addSelectColumn(ServicePeer::SHORT_DESCRIPTION);

		$criteria->addSelectColumn(ServicePeer::DESCRIPTION);

		$criteria->addSelectColumn(ServicePeer::PICTURE);

		$criteria->addSelectColumn(ServicePeer::EXTRA);
	...
	public static function doValidate(Service $obj, $cols = null)
	{
		...
		if ($obj->isNew() || $obj->isColumnModified(ServicePeer::NAME))
			$columns[ServicePeer::NAME] = $obj->getName();
	...
	static function extList(Criteria $oCrit, $iStart=0, $iLimit=-1, $sSort=null, $sDir=null, PropelPDO $con=null )
	{
		...
		if(is_array($aData))
		{
			foreach($aData as $aRow)
			{
				...
				array_push($aResult,ServicePeer::extAddRow($aNewRow));
			}
		}
	...
	static function extConvertObject(Service $oService)
	{
		$aResult['Id'] = $oService->getId();
		$aResult['Index'] = $oService->getIndex();
		$aResult['Name'] = $oService->getName();
		$aResult['ShortDescription'] = $oService->getShortDescription();
		$aResult['Description'] = $oService->getDescription();
		$aResult['Picture'] = $oService->getPicture();
		$aResult['Extra'] = $oService->getExtra();
		return $aResult;
	}
} // BaseServicePeer
