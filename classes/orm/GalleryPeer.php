...
require 'orm/om/BaseGalleryPeer.php';
...
class GalleryPeer extends BaseGalleryPeer {
	...
	static function getAll(Parameter $oParam = null, Sortable $oSortable = null, $iPage = -1,&$oPager = null,$iRows = 20)
	{
		...
		if($oParam instanceof Parameter)
		{
			...
			if($oParam->isExists('haveparent'))
			{
				$oCrit->add(self::PARENT_ID, null, Criteria::ISNOTNULL);
			}
		}
	...
	static function getByParent(Gallery $oCategory = null, Parameter $oParam = null, Sortable $oSortable = null, $iPage = -1,&$oPager = null,$iRows = 20)
	{
		$oCrit = new Criteria();
		$oCrit->addAscendingOrderByColumn(self::INDEX);
		$oCrit->addAscendingOrderByColumn(self::NAME);
	
		if($oCategory instanceof Gallery)
