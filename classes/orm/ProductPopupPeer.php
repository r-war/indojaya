...
require 'orm/om/BaseProductPopupPeer.php';


/**
 * Skeleton subclass for performing query and update operations on the 'product_popup' table.
 *
 * 
 *
 ...
 * @package    orm
 */
class ProductPopupPeer extends BaseProductPopupPeer {
	...
	static function getAll(Parameter $oParam = null, Sortable $oSortable = null, $iPage = -1,&$oPager = null,$iRows = 12)
	{
		...
		if($oParam instanceof Parameter)
		{
			...
			if($oParam->isExists('productdetailid'))
			{
				if($oParam->get('productdetailid') == 'null')
					$oCrit->add(self::PRODUCT_DETAIL_ID, null, Criteria::ISNULL);
				else
					$oCrit->add(self::PRODUCT_DETAIL_ID, $oParam->get('productdetailid'));
			}
