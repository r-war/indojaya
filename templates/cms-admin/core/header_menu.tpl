<ul class="nav">
  ...
  <li class="dropdown {if in_array($oMod->getModule(),array('Content','Page','Menu','News','Service','Event'))}active{/if}">
	...
	<ul class="dropdown-menu">
      ...
      *}  
      ...
      {/if}
      {if in_array('News',$oLoginAdmin->getPrivilegesArray())}
      <li {if $oMod->getModule() == 'News' && $smarty.get.mode == 'article'}class="active"{/if}><a href="{$oMod->getBasePage('News', 'mode=article')}">Articles</a></li>
      {/if}
  ...
  {if $oLoginAdmin->isAuthorized(array('Banner','Gallery'))}
