...
<div class="row-fluid">
	<ul class="breadcrumb">
	    <li>
	    <a href="{$oMod->getBasePage('Gallery')}">{loc k=gallery}</a> <span class="divider">/</span>
	    </li>
	    <li class="active">{$oGallery->getName()}</li>
    </ul>
</div>
...
<tr>
	...
	{include file='core/sortable.tpl' title='#' field='gallery_picture.INDEX' oSortable=$_sortable}
	{include file='core/sortable.tpl' title='picture' field='gallery_picture.PICTURE' oSortable=$_sortable}
	{include file='core/sortable.tpl' title='name' field='gallery_picture.NAME' oSortable=$_sortable}
	<th width="230px">{loc k=options}</th>
</tr>
</thead>
{foreach from=$aGalleryPicture key=idx item=oObj}
