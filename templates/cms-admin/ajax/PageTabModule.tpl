{if $sAjax == 'form'}
	<form id="form" action="{$oMod->getPage($oMod->getModule(),'select='|@cat:$oPageTab->getPrimaryKey())}" method="POST" enctype="multipart/form-data">
	...
	{include file="core/feedback.tpl" source=$oMod->getModule()}
		<div class="form-unit form-horizontal">
		  <div class="control-group">
		     ...
		     <div class="controls">
		       <input type="text" name="oPageTab-index" value="{$oPageTab->getIndex()}" />
		     </div>
		   </div>
		   <div class="control-group">
		     <label class="control-label">{loc k=name}</label>
		     <div class="controls">
		       <input type="text" name="oPageTab-name" id="name" value="{$oPageTab->getName()}" />
		     </div>
		   </div>
		   <div class="control-group">
	...
	<div class="modal-footer">
		...
		{if !$oPageTab->isNew()}<input name="delete" type="button" value="{loc k=delete}" class="btn btn-danger" onclick="doDelete('{$oPageTab->getPrimaryKey()}','{val v=$oPageTab->getName() parsequote=true}')"/>{/if}
